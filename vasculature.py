...
class Bifurcation:
    ...
    def __init__(self, referenceSystem, vectors):
        try:
            ...
            self.points  = [ points.GetPoint(i) for i in range(self.numberOfBranches) ]
            self.vectors = [ pointData.GetArray('BifurcationVectors').GetTuple(i) for i in range(self.numberOfBranches) ]
            self.inPlaneVectors = [ pointData.GetArray('InPlaneBifurcationVectors').GetTuple(i) for i in range(self.numberOfBranches) ]
            
        except AttributeError:
            print("Error building bifurcation! It seems you did not pass me a vtkPolyData.", end='\n')
...
class Vasculature:
    ...
    def _compute_open_centers(self):
        ...
        maxRadius = openBoundariesRefSystem.GetPointData().GetArray(radiusArray).GetRange()[intOne]
    ...
    def _compute_bifurcations_geometry(self):
        ...
        bifurcationsIds = [ bifsIdsArray.GetValue(i) for i in range(self.numberOfBifurcations) ] 
        ...
        if self.numberOfBifurcations > intZero:
            ...
            for index in bifurcationsIds:
                ...
                bifVectors.BifurcationVectors.GetPointData().SetActiveScalars(bifVectors.BifurcationGroupIdsArrayName)
